...
const InvoiceStatusBadge: React.FC<InvoiceStatusBadgeProps> = ({ status, reminderLevel = 0, type }) => {
 ...
 'draft': { bg: 'bg-slate-50 text-slate-600 border-slate-200', icon: <FaFileAlt className="text-xs" />, label: 'Entwurf' },
 'issued': { bg: 'bg-indigo-50 text-indigo-700 border-indigo-200', icon: <FaStamp className="text-xs" />, label: 'Ausgestellt' },
 'paid': { bg: 'bg-emerald-50 text-emerald-700 border-emerald-200', icon: <FaCheckCircle className="text-xs" />, label: 'Bezahlt' },
 'overdue': { bg: 'bg-red-50 text-red-700 border-red-200', icon: <FaExclamationTriangle className="text-xs" />, label: 'Überfällig' },
 'pending': { bg: 'bg-amber-50 text-amber-700 border-amber-200', icon: <FaFileAlt className="text-xs" />, label: 'Offen' },
 'cancelled': { bg: 'bg-slate-100 text-slate-600 border-slate-300', icon: <FaBan className="text-xs" />, label: 'Storniert' },
 'deleted': { bg: 'bg-slate-50 text-slate-500 border-slate-200', icon: <FaTrashAlt className="text-xs" />, label: 'Gelöscht' },
 'archived': { bg: 'bg-slate-800 text-white border-slate-700', icon: <FaArchive className="text-xs" />, label: 'Archiviert' },
 'reminder': { bg: 'bg-orange-50 text-orange-800 border-orange-200 shadow-sm animate-pulse-subtle', icon: <FaBell className="text-xs" />, label: 'Mahnung' },
 };
 ...
 return (
 <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-sm text-xs font-medium border tracking-tight transition-all ${entry.bg}`}>
 {entry.icon}
 {getLabel()}
 </span>
 );
