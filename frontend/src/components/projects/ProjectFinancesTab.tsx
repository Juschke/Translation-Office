...
const ProjectFinancesTab = ({
 ...
 {renderEditableCell(pos.id, 'description', pos.description, 'text', 'font-medium text-slate-700 w-full bg-transparent outline-none focus:bg-white focus:ring-2 focus:ring-brand-100 rounded px-1 -mx-1')}
 </td>
 ...
 className="text-right text-xs font-medium text-red-300 bg-transparent outline-none cursor-pointer hover:text-red-500 transition-colors appearance-none"
 >
 ...
 <span className="text-xs font-medium text-emerald-700">Marge / Gewinn</span>
 <span className="text-lg font-semibold text-emerald-700">{financials.margin.toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} €</span>
 </div>
 ...
 <span className="text-xs font-semibold text-purple-600">Rechnung verknüpft</span>
 <span className={clsx('text-xs font-medium px-2 py-0.5 rounded-full', {
 'bg-slate-100 text-slate-600': activeInvoice.status === 'draft',
 'bg-indigo-100 text-indigo-700': activeInvoice.status === 'issued',
 'bg-blue-100 text-blue-700': activeInvoice.status === 'sent',
 'bg-emerald-100 text-emerald-700': activeInvoice.status === 'paid',
 'bg-red-100 text-red-700': activeInvoice.status === 'overdue',
 })}>
 ...
 <div className="text-sm font-medium text-slate-800">{activeInvoice.invoice_number}</div>
 ...
 className="w-full py-2 bg-purple-600 text-white rounded-sm text-sm font-medium hover:bg-purple-700 transition flex items-center justify-center gap-2"
 >
