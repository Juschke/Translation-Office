...
const ProjectFinancialSidebar = ({
    ...
    <div className="w-full lg:w-72 bg-white border border-slate-200 rounded-lg shadow-sm font-sans divide-y divide-slate-100 flex flex-col">
        ...
        <div className="p-5 space-y-3">
            <h4 className="text-sm font-bold text-slate-400 mb-4">Kalkulation</h4>
            ...
            <div className="space-y-2">
                ...
                {(isCertified || hasApostille || isExpress || classification === 'ja' || copies > 0) && (
                    <div className="space-y-1.5 pl-1.5 border-l border-slate-100">
                        {isCertified && <div className="flex justify-between text-sm text-slate-400"><span>Beglaubigung</span><span>5,00 €</span></div>}
                        {hasApostille && <div className="flex justify-between text-sm text-slate-400"><span>Apostille</span><span>15,00 €</span></div>}
                        {isExpress && <div className="flex justify-between text-sm text-slate-400"><span>Express</span><span>15,00 €</span></div>}
                        {classification === 'ja' && <div className="flex justify-between text-sm text-slate-400"><span>Klassifizierung</span><span>15,00 €</span></div>}
                        {copies > 0 && <div className="flex justify-between text-sm text-slate-400"><span>Kopien ({copies}x)</span><span>{(copies * Number(copyPrice)).toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} €</span></div>}
                    </div>
                )}
            </div>
            ...
            <div className="pt-3 space-y-1.5 border-t border-slate-50">
                ...
                <div className="flex justify-between items-baseline pt-2">
                    <span className="text-[11px] font-bold text-slate-800">Gesamt</span>
        ...
        <div className="p-5 space-y-3 bg-slate-50/30">
            <div className="flex justify-between items-center mb-1">
                <span className="text-sm font-bold text-slate-400">Zahlung</span>
            ...
            <div className="space-y-2">
                ...
                <div className={clsx("flex flex-col gap-0.5 p-3 rounded-md border", remainingBalance <= 0.01 ? "bg-emerald-50 border-emerald-100 text-emerald-700" : "bg-white border-slate-100 text-slate-700")}>
        ...
        <div className="p-5 space-y-3">
            <div className="flex justify-between items-center">
                <span className="text-sm font-bold text-slate-400">Projektmarge</span>
                <span className={clsx("text-sm font-bold px-1.5 py-0.5 rounded", profit >= 0 ? "bg-slate-100 text-slate-600" : "bg-red-50 text-red-600")}>
            ...
            <div className="w-full bg-slate-100 rounded-full h-1 overflow-hidden">
                <div
                    className={clsx('h-full transition-all duration-700', profitMargin > 40 ? 'bg-emerald-500' : profitMargin > 20 ? 'bg-slate-500' : 'bg-amber-400',)}
