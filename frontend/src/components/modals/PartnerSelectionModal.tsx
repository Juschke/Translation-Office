...
const PartnerSelectionModal: React.FC<PartnerSelectionModalProps> = ({ isOpen, onClose, onSelect }) => {
 ...
 <tr>
 <th className="px-6 py-2.5 text-xs font-semibold text-slate-400 cursor-pointer hover:text-teal-700 transition" onClick={() => handleSort('name')}>
 ...
 <div className="font-medium text-slate-900 text-sm group-hover:text-teal-700 transition leading-tight">{p.name}</div>
 ...
 className="bg-teal-700 text-white px-4 py-1.5 rounded text-xs font-semibold shadow-sm hover:bg-teal-800 transition"
 >
 Übernehmen
 </button>
 </td>
 ...
 {mode !== 'list' && (
 <button onClick={handleSave} className="px-10 py-2.5 bg-teal-700 text-white rounded text-sm font-medium shadow-sm hover:bg-teal-800 transition">
 Speichern
 </button>
 )}
 </div>
