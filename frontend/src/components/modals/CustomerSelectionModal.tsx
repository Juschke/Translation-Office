...
const CustomerSelectionModal: React.FC<CustomerSelectionModalProps> = ({ isOpen, onClose, onSelect }) => {
 ...
 color: 'bg-slate-50 text-slate-900', // Default color for now, could be dynamic
 type: c.type || 'private',
 ...
 return sortConfig.direction === 'asc' ? <FaSortUp className="text-slate-700 ml-1" size={10} /> : <FaSortDown className="text-slate-700 ml-1" size={10} />;
 };
 ...
 <tr>
 <th className="px-6 py-2.5 text-xs font-semibold text-slate-400 cursor-pointer hover:text-slate-900 transition" onClick={() => handleSort('name')}>
 <div className="flex items-center">Name / Firma {getSortIcon('name')}</div>
 </th>
 <th className="px-4 py-2.5 text-xs font-semibold text-slate-400 w-40">Typ</th>
 ...
 <span className="text-xs font-semibold text-slate-400 block">Ansprechpartner</span>
 <div className="font-medium text-slate-800">{c.contact}</div>
 </div>
 <div className="space-y-1">
 <span className="text-xs font-semibold text-slate-400 block">Telefon</span>
