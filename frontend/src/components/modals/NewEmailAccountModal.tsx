...
const NewEmailAccountModal: React.FC<NewEmailAccountModalProps> = ({ isOpen, onClose, onSubmit, initialData }) => {
   const [formData, setFormData] = useState({
      name: 'ITC-KS Noreply',
      email: 'noreply@itc-ks.com',
      smtp_host: 'mail.manitu.de',
      smtp_port: 465,
      smtp_encryption: 'ssl',
      imap_host: 'mail.manitu.de',
      imap_port: 993,
      imap_encryption: 'ssl',
      username: 'noreply@itc-ks.com',
      password: 'Tjumenka1966!',
      is_default: true,
   ...
   return (
      <div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center backdrop-blur-sm p-4">
         <div className="bg-white rounded-sm shadow-sm w-full max-w-3xl max-h-[85vh] flex flex-col overflow-hidden animate-fade-in-up transform transition-all">
            <form onSubmit={handleSave} className="flex flex-col h-full overflow-hidden">
               ...
               <div className="flex-1 overflow-y-auto p-8 custom-scrollbar space-y-10 bg-white">
                  ...
                  <div className="space-y-4">
                     <label className="block text-xs font-medium text-slate-700 mb-3 flex items-center gap-2">
                        <FaServer className="text-slate-700" /> Ausgangsserver (SMTP)
                     </label>
                  ...
                  <div className="space-y-4">
                     <label className="block text-xs font-medium text-slate-700 mb-3 flex items-center gap-2">
                        <FaServer className="text-slate-700" /> Eingangsserver (IMAP)
                     </label>
                  ...
                  <div className="space-y-4">
                     <label className="block text-xs font-medium text-slate-700 mb-3 flex items-center gap-2">
                        <FaKey className="text-slate-700" /> Authentifizierung
                     </label>
                  ...
                  <div className="space-y-4 pb-8">
                     ...
                     <div className="flex items-center gap-4 p-4 bg-slate-50 border border-slate-200 rounded-sm">
                        <Switch
                           id="is_default"
                        ...
                        <label htmlFor="is_default" className="text-xs font-medium text-slate-700 cursor-pointer">
                           ...
                           <span className="block text-[10px] text-slate-400 font-normal mt-0.5 tracking-tight uppercase">
               ...
               <div className="bg-slate-50 px-6 py-4 border-t border-slate-200 flex justify-between items-center shrink-0">
                  <Button
                     ...
                     variant="ghost"
                     onClick={onClose}
                     className="px-6 h-10 text-slate-500 text-[11px] font-bold tracking-widest hover:bg-white transition rounded-sm border border-slate-200"
                  >
                     ABBRECHEN
                  </Button>
                  <Button
                     type="submit"
                     className="px-8 h-10 bg-gradient-to-b from-[#235e62] to-[#1B4D4F] hover:from-[#2a7073] hover:to-[#235e62] text-white rounded-sm text-[11px] font-bold tracking-widest shadow-lg border border-[#123a3c] transition-all"
                  >
                     KONTO SPEICHERN
                  </Button>
               </div>
