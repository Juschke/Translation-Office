...
const NewEmailAccountModal: React.FC<NewEmailAccountModalProps> = ({ isOpen, onClose, onSubmit, initialData }) => {
   ...
   return (
      <div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center backdrop-blur-sm p-4">
         <div className="bg-white rounded-sm shadow-sm w-full max-w-3xl max-h-[85vh] flex flex-col overflow-hidden animate-fadeInUp">
            <form onSubmit={handleSave} className="flex flex-col h-full overflow-hidden">
               ...
               <div className="bg-white px-6 py-3 border-b border-slate-200 flex justify-between items-center shrink-0">
                  <h3 className="font-semibold text-base text-slate-800 flex items-center gap-3">
                     {initialData ? 'E-Mail Konto bearbeiten' : 'Neues E-Mail Konto'}
               ...
               <div className="flex-1 overflow-y-auto p-8 custom-scrollbar space-y-10 bg-white">
                  ...
                  <div className="space-y-4">
                     <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                        <FaUser className="text-slate-500" /> Konto-Details
                     </label>
                  ...
                  <div className="space-y-4">
                     <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                        <FaServer className="text-slate-500" /> Ausgangsserver (SMTP)
                     </label>
                  ...
                  <div className="space-y-4">
                     <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                        <FaServer className="text-slate-500" /> Eingangsserver (IMAP)
                     </label>
                  ...
                  <div className="space-y-4">
                     <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                        <FaKey className="text-slate-500" /> Authentifizierung
                     </label>
                  ...
                  <div className="space-y-4 pb-8">
                     <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                        <FaInfoCircle className="text-slate-500" /> Einstellungen
                     </label>
