...
const NewProjectModal: React.FC<NewProjectModalProps> = ({ isOpen, onClose, onSubmit, initialData, isLoading }) => {
 ...
 <div className="w-6 h-6 rounded bg-slate-50 text-slate-900 flex items-center justify-center text-xs font-medium">01</div>
 ...
 <label className="block text-sm font-medium text-slate-500 mb-1 ml-1 cursor-pointer" onClick={() => datePickerRef.current?.setFocus()}>Lieferdatum</label>
 <div
 className="relative h-10 bg-white border border-slate-300 rounded shadow-sm overflow-hidden flex items-center cursor-pointer hover:border-slate-400 focus-within:border-slate-900 focus-within:ring-2 focus-within:ring-slate-950/10 transition-all"
 onClick={() => {
 datePickerRef.current?.setFocus();
 (datePickerRef.current as any)?.setOpen?.(true);
 }}
 >
 <div className="absolute left-3 text-slate-400 pointer-events-none group-focus-within:text-slate-600 transition-colors">
 ...
 locale="de"
 className="w-full h-full bg-transparent border-none pl-9 pr-3 py-1.5 text-sm font-medium text-slate-700 outline-none cursor-pointer rounded"
 ...
 <div className="space-y-4">
 <div className="flex items-center gap-3 pb-2 border-b border-slate-100">
 <div className="w-6 h-6 rounded bg-slate-50 text-slate-900 flex items-center justify-center text-xs font-medium">02</div>
 ...
 <button onClick={() => setShowPartnerModal(true)} className="h-10 px-4 bg-slate-900 text-white rounded-r border-l-0 hover:bg-slate-800 transition flex items-center gap-2 shadow-sm"><FaPlus className="text-xs" /> <span className="text-xs font-medium">NEU</span></button>
 </div>
 </div>
 </div>
 ...
 {payments.length === 0 ? (
 <div className="text-xs text-slate-400 italic py-2 text-center border-2 border-dashed border-slate-50 rounded">Keine Zahlungen erfasst.</div>
 ) : (
 ...
 <div className="space-y-4">
 <div className="flex items-center gap-2 mb-2 pb-2 border-b border-slate-200"><FaInfoCircle className="text-slate-400 text-xs" /><h4 className="text-xs font-medium text-slate-500">Meta Info</h4></div>
 <div className="space-y-2 text-xs">
 <div className="flex justify-between"><span className="text-slate-500">Erstellt:</span><span className="font-medium text-slate-700">{creationDate}</span></div>
 <div className="flex justify-between"><span className="text-slate-500">Manager:</span><span className="font-medium text-slate-700">{projectManager}</span></div>
 </div>
 </div>
 ...
 {isCertified && (
 <div className="flex justify-between text-sm text-slate-400 pl-2"><span>+ Beglaubigung</span><span>5,00 €</span></div>
 )}
 {hasApostille && (
 <div className="flex justify-between text-sm text-slate-400 pl-2"><span>+ Apostille</span><span>15,00 €</span></div>
 )}
 {isExpress && (
 <div className="flex justify-between text-sm text-slate-400 pl-2"><span>+ Express-Zuschlag</span><span>15,00 €</span></div>
 )}
 {classification === 'ja' && (
 <div className="flex justify-between text-sm text-slate-400 pl-2"><span>+ FS-Klassifizierung</span><span>15,00 €</span></div>
 )}
 {copies > 0 && (
 ...
 <div className="pt-2 border-t border-slate-100 flex justify-between font-medium text-slate-800"><span>Gesamt Netto</span><span>{calcNet.toFixed(2)} €</span></div>
 <div className="flex justify-between text-xs text-slate-400"><span>MwSt. 19%</span><span>{calcTax.toFixed(2)} €</span></div>
 <div className="pt-2 border-t-2 border-slate-100 flex justify-between text-xl font-medium text-slate-900 transition-all"><span>Gesamt</span><span>{calcGross.toFixed(2)} €</span></div>
 {totalPaid > 0 && (
 ...
 <span className={clsx("text-xs font-medium", profit >= 0 ? "text-slate-800" : "text-red-600")}>{profit.toFixed(2)} €</span>
 </div>
 <div className="w-full bg-slate-300 rounded-full h-2 overflow-hidden shadow-inner">
 <div className={clsx("h-full transition-all duration-500", profitMargin > 40 ? "bg-emerald-500" : profitMargin > 20 ? "bg-slate-700" : "bg-amber-500")} style={{ width: `${Math.min(100, Math.max(0, profitMargin))}%` }}></div>
 </div>
 <div className="text-right mt-1.5 text-xs font-medium text-slate-500">{profitMargin.toFixed(1)}% Marge</div>
 </div>
 </div>
 </div>
 </div>
