...
const FileUploadModal = ({ isOpen, onClose, onUpload }: FileUploadModalProps) => {
 ...
 {/* Footer */}
 <div className="px-6 py-4 border-t border-slate-100 flex justify-end gap-3 bg-transparent">
 <button onClick={onClose} disabled={uploading} className="px-5 py-2 text-xs font-semibold text-slate-500 hover:text-slate-800 transition-colors disabled:opacity-30">Abbrechen</button>
 <button
 disabled={selectedFiles.length === 0 || !selectedFiles.some(f => f.isValid) || uploading}
 onClick={handleUpload}
 className="px-6 py-2 bg-slate-900 text-white rounded text-xs font-semibold hover:bg-slate-800 transition shadow-sm disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
 >
