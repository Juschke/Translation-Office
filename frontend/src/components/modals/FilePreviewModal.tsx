            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl h-[85vh] flex flex-col overflow-hidden">
                ...
                <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                    <div className="flex flex-col">
                        <h2 className="text-sm font-black text-slate-800 uppercase tracking-widest truncate max-w-md" title={file.name}>
                            {file.name}
                        </h2>
                        <span className="text-[10px] text-slate-400 font-medium">Vorschau-Modus</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <button
                            onClick={onDownload}
                            className="px-4 py-2 bg-brand-50 text-brand-700 hover:bg-brand-100 rounded text-[10px] font-black uppercase flex items-center gap-2 transition"
                        >
                            <FaDownload /> Herunterladen
                        </button>
                        <button
                            onClick={() => window.open(file.url, '_blank')}
                            className="px-4 py-2 bg-slate-100 text-slate-600 hover:bg-slate-200 rounded text-[10px] font-black uppercase flex items-center gap-2 transition"
                        >
                ...
                <div className="flex-1 bg-slate-100 relative overflow-hidden flex items-center justify-center p-4">
                    ...
                    {!isImage && !isPDF && !isWord && (
                        <div className="text-center p-10 bg-white rounded-xl shadow-sm border border-slate-200 max-w-md">
                            ...
                            <h3 className="font-bold text-slate-700 mb-2">Keine Vorschau verfügbar</h3>
