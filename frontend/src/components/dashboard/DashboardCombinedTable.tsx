...
const DashboardCombinedTable: React.FC<DashboardCombinedTableProps> = ({
                    ...
                    >
                        <td className="px-5 py-3">
                            <div className="flex flex-col">
                                <span className="text-xs font-semibold text-slate-800 group-hover:text-slate-900 transition tracking-tight">
                                    {invoice.number}
                                </span>
        ...
        return data.map((task) => {
            ...
            return (
                ...
                >
                    <td className="px-5 py-3">
                        <div className="flex flex-col">
                            <span className="text-xs font-semibold text-slate-800 group-hover:text-slate-900 transition tracking-tight truncate max-w-[200px]">
                                {task.name}
                            </span>
                    ...
                    <td className="px-5 py-3">
                        <div className="flex flex-col">
                            ...
                            <div className="flex items-center gap-1.5 mt-0.5">
                                <span className="text-xs font-semibold text-slate-400">{statusInfo.label}</span>
                            </div>
                        </div>
                    </td>
                    <td className="px-5 py-3 text-right">
                        {deadline ? (
                            <div className="flex flex-col items-end">
                                <span className={clsx("text-xs font-semibold tabular-nums", isUrgent ? "text-red-500" : "text-slate-800")}>
                                    {deadline.toLocaleDateString('de-DE', { day: '2-digit', month: '2-digit', year: '2-digit' })}
                                </span>
                                <span className={clsx("text-xs font-medium", isUrgent ? "text-red-400" : "text-slate-400")}>
    ...
    const hasActiveTabData = () => {
        if (activeTab === 'tasks') return activeTasks.length > 0;
        if (activeTab === 'quotes') return openQuotes.length > 0;
        if (activeTab === 'deliver') return readyToDeliver.length > 0;
        if (activeTab === 'overdue') return overdueInvoices.length > 0;
        return false;
    };
                    ...
                    {tabs.map((tab) => {
                        ...
                        return (
                            <button
                                ...
                                className={clsx(
                                    "flex items-center gap-2 py-4 text-sm font-medium transition-all relative whitespace-nowrap border-b-2",
                                    isActive
            ...
            <div className="overflow-auto flex-1 custom-scrollbar flex flex-col">
                {hasActiveTabData() ? (
                    <table className="w-full text-left">
                        <thead className="bg-transparent text-slate-500 text-xs font-semibold sticky top-0 z-10">
                            <tr>
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center p-8 text-center animate-fadeIn">
                        ...
                        <span className="text-sm font-bold text-slate-600">Aktuell nichts zu tun</span>
                        <p className="text-[11px] text-slate-400 mt-1 max-w-[200px]">Alle Elemente in diesem Tab sind auf dem neuesten Stand.</p>
                    </div>
                )}
