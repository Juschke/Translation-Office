    ...
    ({ className, label, error, helperText, containerClassName, ...props }, ref) => {
        ...
        return (
            <div className={cn("w-full", containerClassName)}>
                ...
                <textarea
                    className={cn(
                        "flex min-h-[100px] w-full bg-white px-3 py-2 text-sm font-semibold text-slate-800 shadow-sm transition-all resize-none",
                ...
                />
                {helperText && (
                    <p className={cn("mt-1.5 text-[10px] font-medium ml-1", error ? "text-red-600" : "text-slate-400")}>
                        {helperText}
                    </p>
                )}
            </div>
