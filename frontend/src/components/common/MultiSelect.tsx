                ...
                <div className="absolute z-50 w-full bg-white border border-slate-200 max-h-60 overflow-hidden flex flex-col">
                    ...
                    <div className="overflow-y-auto custom-scrollbar flex-1">
                        {filteredOptions.map((option, index) => (
                            <div
                                key={option.value}
                                className={`px-4 py-2.5 text-sm cursor-pointer transition flex items-center gap-2 ${activeIndex === index ? 'bg-slate-50 text-brand-700' : ''
                                    } ${value.includes(option.value) ? 'bg-brand-50 text-brand-700 font-medium' : 'text-slate-600 hover:bg-slate-50'
                                    }`}
