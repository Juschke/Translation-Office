...
const UniversalForm: React.FC<UniversalFormProps> = ({
 ...
 {title && <h5 className="text-xs font-semibold text-slate-400">{title}</h5>}
 {onCancel && (
 <button
 type="button"
 onClick={onCancel}
 className="text-xs text-red-500 font-semibold flex items-center gap-1 hover:text-red-700 transition"
 >
 <FaTimes /> Abbrechen
 </button>
 )}
 ...
 type="submit"
 disabled={isLoading}
 className="w-full h-11 bg-slate-900 text-white rounded-sm text-xs font-semibold hover:bg-slate-800 transition shadow-sm flex items-center justify-center gap-2 active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed"
 >
 {isLoading ? 'Speichern...' : (
 <>
 {submitLabel}
