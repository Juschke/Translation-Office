...
const CommunicationHub = () => {
    ...
    // Data Queries — nur der aktive Ordner wird geladen (enabled-Flag spart 2 von 3 Requests)
    ...
    const { data: archiveMessages = [], isLoading: isLoadingArchive } = useQuery({
    ...
    const isLoadingActive = (activeTab === 'inbox' && isLoadingInbox)
            ...
            <div className="flex flex-col bg-white border border-slate-200 shadow-sm rounded-sm overflow-hidden" style={{ height: 'calc(100vh - 180px)', minHeight: '500px' }}>
                <div className="flex-1 flex overflow-hidden">
                    ...
                    <div className="flex-1 overflow-hidden flex flex-col bg-white">
                        {['inbox', 'sent', 'archive', 'trash'].includes(activeTab) && (
                            <div className="flex-1 flex min-h-0 overflow-hidden">
                                <div className={clsx(
                                    ...
                                    {selectedMails.length > 0 && (
                                        <div className="bg-slate-50 border-b border-slate-200">
                                            <BulkActions
                                                ...
                                                actions={[
                                                    {
                                                        label: activeTab === 'trash' ? 'Wiederherstellen' : 'Archivieren',
                                                        icon: <FaSyncAlt className="text-xs" />,
                                                        onClick: () => {
                                                            if (activeTab === 'trash') restoreMailsMutation.mutate(selectedMails);
                                                            else archiveMailsMutation.mutate(selectedMails);
                                                        },
                                                        variant: 'default',
                                                        show: activeTab !== 'archive' && activeTab !== 'sent'
                                                    },
                                                    {
                                                        label: activeTab === 'trash' ? 'Endgültig löschen' : 'Löschen',
                                    ...
                                    <MailListPanel
                                        mails={activeTab === 'inbox' ? inboxMessages : activeTab === 'sent' ? sentMessages : activeTab === 'archive' ? archiveMessages : trashMessages}
                                        folder={activeTab}
                                        ...
                                        onSelectAll={() => {
                                            const currentMails = activeTab === 'inbox' ? inboxMessages : activeTab === 'sent' ? sentMessages : activeTab === 'archive' ? archiveMessages : trashMessages;
                                            setSelectedMails(selectedMails.length === currentMails.length ? [] : currentMails.map((m: any) => m.id));
                                        }}
                                        onArchive={(id: number) => archiveMailsMutation.mutate([id])}
                                        onRestore={(id: number) => restoreMailsMutation.mutate([id])}
                                ...
                                {viewingMail && (
                                    <div className="flex-1 flex flex-col min-w-0 bg-white">
                                        <MailDetailPanel
                                            ...
                                            onArchive={(id: any) => archiveMailsMutation.mutate([id])}
                                            onRestore={(id: any) => restoreMailsMutation.mutate([id])}
