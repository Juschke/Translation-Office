...
const GuestProjectView = () => {
    ...
    return (
        <div className="h-screen flex flex-col bg-slate-50/50 font-sans text-slate-600 overflow-hidden">
            {/* Fixed Header */}
                            ...
                            {STEPS.map((step, idx) => {
                                ...
                                return (
                                    <div key={idx} className="flex flex-col items-center gap-1 bg-white px-2">
                                        <div className={clsx(
                                            "w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ring-4 ring-white transition-all duration-500",
                                            isActive ? "bg-brand-600 text-white shadow-lg shadow-brand-500/30" : "bg-slate-100 text-slate-400"
                                        )}>
                                            {isActive ? <FaCheck /> : idx + 1}
                                        </div>
                                        <span className={clsx(
                                            "text-[9px] font-bold uppercase tracking-wider transition-colors",
                                            isCurrent ? "text-brand-700" : isActive ? "text-brand-600/70" : "text-slate-300"
                                        )}>{step.label}</span>
                                    </div>
                                );
                            })}
                            ...
                            <div className="bg-white p-4 rounded-sm border border-slate-100 shadow-sm h-full">
                                ...
                                <div className="space-y-2 text-sm">
                                    {!isEditingCustomer ? (
                                        <>
                                            <div>
                                                <span className="block text-[10px] text-slate-400 font-bold uppercase">Name / Firma</span>
                                                <span className="font-bold text-slate-800">{project.customer.company_name || `${project.customer.first_name} ${project.customer.last_name}`}</span>
                                            </div>
                                            <div>
                                                <span className="block text-[10px] text-slate-400 font-bold uppercase">Anschrift</span>
                                            ...
                                            <div className="flex gap-4">
                                                {project.customer.email && (
                                                    <div>
                                                        <span className="block text-[10px] text-slate-400 font-bold uppercase">E-Mail</span>
                                                        <a href={`mailto:${project.customer.email}`} className="text-brand-600 hover:underline">{project.customer.email}</a>
                                                    </div>
                                                )}
                                                {project.customer.phone && (
                                                    <div>
                                                        <span className="block text-[10px] text-slate-400 font-bold uppercase">Telefon</span>
                            ...
                            <div className="flex-1 flex flex-col justify-center gap-4">
                                <div className="flex items-center justify-between bg-slate-50 p-3 rounded-sm">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-xs font-bold text-slate-700">{project.target_lang?.name_native || project.target_lang?.name_internal || 'Englisch'}</span>
                                    </div>
                                </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <table className="w-full text-sm">
                                    ...
                                    <tfoot className="border-t border-slate-100 font-bold text-slate-800">
                                        <tr>
                                ...
                                {project.tenant ? (
                                    <div className="space-y-1 text-xs text-slate-600">
                                        <div className="flex justify-between border-b border-slate-200 pb-1">
                                            ...
                                            <span className="font-bold">{project.tenant.company_name}</span>
                                        </div>
                                        <div className="flex justify-between border-b border-slate-200 pb-1">
                                            <span className="text-slate-400">Bank</span>
                                            <span className="font-bold">{project.tenant.bank_name || '-'}</span>
                                        </div>
                                        <div className="flex justify-between border-b border-slate-200 pb-1">
                                            <span className="text-slate-400">IBAN</span>
                                            <span className="font-mono font-bold select-all">{project.tenant.bank_iban || '-'}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span className="text-slate-400">BIC</span>
                                            <span className="font-mono font-bold select-all">{project.tenant.bank_bic || '-'}</span>
                                        </div>
                                    </div>
                                ) : (
                                ...
                                {targetFiles.map((file: any) => (
                                    <div key={file.id} className="flex items-center justify-between p-2 bg-white rounded-sm border border-brand-50 shadow-sm hover:shadow-md transition cursor-pointer">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div className="font-bold text-sm text-slate-700">{file.file_name}</div>
                                                <div className="text-[10px] text-slate-400">{(file.file_size / 1024).toFixed(1)} KB</div>
                                            </div>
                                        </div>
                                        <button className="text-brand-600 hover:text-brand-800"><FaDownload /></button>
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    <div className="bg-white rounded-sm border border-slate-100 shadow-sm overflow-hidden flex flex-col h-[400px]">
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar bg-white flex flex-col-reverse">
                            ...
                            {[...(project.messages || [])].sort((a: any, b: any) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).map((msg: any) => (
                                <div key={msg.id} className={clsx("flex flex-col max-w-[85%]", !msg.user_id ? "self-end items-end" : "self-start items-start")}>
                                    <div className={clsx("px-3 py-2 rounded-2xl text-xs shadow-sm", !msg.user_id ? "bg-brand-600 text-white rounded-br-none" : "bg-slate-100 text-slate-700 rounded-bl-none")}>
                                        {msg.content}
                                    </div>
                        ...
                        <div className="p-3 border-t border-slate-100 bg-slate-50/50">
                            <div className="flex gap-2">
                                ...
                                <button onClick={() => cameraInputRef.current?.click()} className="w-8 h-8 flex items-center justify-center rounded-full bg-slate-100 border border-slate-200 text-slate-500 hover:text-brand-600 transition shadow-sm">
                                    <FaCamera />
                                </button>
                                <button onClick={() => fileInputRef.current?.click()} className="w-8 h-8 flex items-center justify-center rounded-full bg-white border border-slate-200 text-slate-400 hover:text-brand-600 transition shadow-sm">
                                    <FaPaperclip />
                                </button>
                                ...
                                <div className="flex-1 relative">
                                    <input
                                        ...
                                        className="w-full h-8 pl-3 pr-10 rounded-full border border-slate-200 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none shadow-sm text-xs"
                                        onKeyDown={(e) => {
                                            if (e.key === 'Enter') handleSendMessage();
                                        }}
                                    />
                                    <button onClick={handleSendMessage} className="absolute right-1 top-1 w-6 h-6 bg-brand-600 text-white rounded-full flex items-center justify-center hover:bg-brand-700 transition">
            ...
            <div className="bg-white border-t border-slate-200 py-3 flex-none z-20">
                <div className="max-w-3xl mx-auto px-4 text-center text-[10px] text-slate-400">
                    <div className="flex flex-wrap justify-center gap-4 mb-1 font-bold uppercase tracking-wider">
                        <button onClick={() => setActiveModal('imprint')} className="hover:text-brand-600 transition">Impressum</button>
                        <button onClick={() => setActiveModal('privacy')} className="hover:text-brand-600 transition">Datenschutz</button>
                        <button onClick={() => setActiveModal('terms')} className="hover:text-brand-600 transition">AGB</button>
                        <button onClick={() => setActiveModal('avv')} className="hover:text-brand-600 transition flex items-center gap-1"><FaFileContract /> Auftragsverarbeitung</button>
                    </div>
                    ...
                    <div className="bg-white rounded-sm shadow-2xl w-full max-w-3xl max-h-[85vh] overflow-y-auto relative p-8">
                        ...
                        {activeModal === 'imprint' && (
                            <div className="prose prose-sm max-w-none text-slate-700">
                                ...
                                <div className="mb-6">
                                    <h3 className="font-bold text-slate-800 mb-1">{tInfos.name}</h3>
                                    <p>{tInfos.street} {tInfos.houseNo}</p>
                                    <p>{tInfos.zip} {tInfos.city}</p>
                                </div>

                                <div className="mb-6">
                                    <h3 className="font-bold text-slate-800 mb-1">Vertreten durch:</h3>
                                    <p>{tInfos.owner}</p>
                                </div>
                                ...
                                <div className="mb-6">
                                    <h3 className="font-bold text-slate-800 mb-1">Umsatzsteuer-Identifikationsnummer:</h3>
                                    <p>gemäß § 27 a Umsatzsteuergesetz: <br />{tInfos.vatId}</p>
                                </div>

                                <div className="mb-6">
                                    <h3 className="font-bold text-slate-800 mb-1">Steuernummer:</h3>
                                    <p>{tInfos.taxNum}</p>
                                </div>

                                <div className="mb-6">
                                    <h3 className="font-bold text-slate-800 mb-1">Verantwortlich für den Inhalt nach § 55 Abs. 2 RStV:</h3>
                                    <p>{tInfos.owner}</p>
                                    <p>{tInfos.street} {tInfos.houseNo}</p>
                                    <p>{tInfos.zip} {tInfos.city}</p>
                                </div>
                                ...
                                <div className="mb-6">
                                    <h3 className="font-bold text-slate-800 mb-1">Streitschlichtung</h3>
                                ...
                                <div className="mb-6">
                                    <h3 className="font-bold text-slate-800 mb-1">Haftung für Inhalte</h3>
                        ...
                        {activeModal === 'privacy' && (
                            <div className="prose prose-sm max-w-none text-slate-700">
                                ...
                                <h3 className="font-bold text-slate-800 mt-6 mb-2">2. Allgemeine Hinweise und Pflichtinformationen</h3>
                                <p className="font-bold">Datenschutz</p>
                                <p>Wir nehmen den Schutz Ihrer persönlichen Daten sehr ernst. Wir behandeln Ihre personenbezogenen Daten vertraulich und entsprechend der gesetzlichen Datenschutzvorschriften sowie dieser Datenschutzerklärung.</p>

                                <p className="font-bold mt-2">Hinweis zur verantwortlichen Stelle</p>
                                ...
                                <h3 className="font-bold text-slate-800 mt-6 mb-2">3. Datenerfassung durch unser Kundenportal</h3>
                                ...
                                <h3 className="font-bold text-slate-800 mt-6 mb-2">4. Ihre Rechte</h3>
                                ...
                                <h3 className="font-bold text-slate-800 mt-6 mb-2">5. Datensicherheit</h3>
                        ...
                        {activeModal === 'terms' && (
                            <div className="prose prose-sm max-w-none text-slate-700">
                                ...
                                <h3 className="font-bold text-slate-800 mt-6 mb-2">§ 2 Vertragsschluss</h3>
                                ...
                                <h3 className="font-bold text-slate-800 mt-6 mb-2">§ 3 Mitwirkungspflicht des Auftraggebers</h3>
                                ...
                                <h3 className="font-bold text-slate-800 mt-6 mb-2">§ 4 Ausführung und Mängelbeseitigung</h3>
                                ...
                                <h3 className="font-bold text-slate-800 mt-6 mb-2">§ 5 Haftung</h3>
                                ...
                                <h3 className="font-bold text-slate-800 mt-6 mb-2">§ 6 Zahlungsbedingungen</h3>
                                ...
                                <h3 className="font-bold text-slate-800 mt-6 mb-2">§ 7 Eigentumsvorbehalt und Urheberrecht</h3>
                                ...
                                <h3 className="font-bold text-slate-800 mt-6 mb-2">§ 8 Schlussbestimmungen</h3>
                            ...
                            <div className="prose prose-sm max-w-none text-slate-700">
                                ...
                                <div className="bg-slate-50 p-6 rounded-sm border border-slate-100 flex items-center justify-between mt-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div className="font-bold text-slate-800">AV-Vertrag Standard</div>
