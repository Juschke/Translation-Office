...
import { FaPlus, FaFileContract, FaEuroSign, FaClock, FaFilter, FaTimes, FaUndo, FaChevronDown } from 'react-icons/fa';
...
const QUOTE_STATUSES = ['offer', 'quote_sent'];
...
const Quotes = () => {
    ...
    const createMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            toast.success(t('quotes.created_success'));
        },
        onError: () => toast.error(t('quotes.created_error')),
    });
    ...
    const baseQuotes = useMemo(() => Array.isArray(projects) ? projects.filter((p: any) => QUOTE_STATUSES.includes(p.status?.toLowerCase())) : [], [projects]);
    const filteredQuotes = useMemo(() => baseQuotes.filter((p: any) => {
        ...
        return true;
    }), [baseQuotes, advancedFilters]);

    const totalValue = baseQuotes.reduce((acc: number, p: any) => acc + parseFloat(p.price_total || 0), 0);
    const recentCount = baseQuotes.filter((p: any) => {
    ...
    const handleExport = (format: 'csv' | 'xlsx' | 'pdf') => {
        ...
        const rows = filteredQuotes.map((p: any) => [p.project_number || p.id, p.project_name, p.customer?.company_name || `${p.customer?.first_name} ${p.customer?.last_name}`, p.source_language?.iso_code || '', p.target_language?.iso_code || '', p.status, p.deadline ? new Date(p.deadline).toLocaleDateString('de-DE') : '', p.price_total || '0']);
        const csvContent = [headers, ...rows].map(e => e.join(';')).join('\n');
        const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
        triggerBlobDownload(blob, `quotes_export_${new Date().toISOString().split('T')[0]}.${format === 'xlsx' ? 'csv' : format}`);
    };
    ...
    return (
        <div className="flex-1 flex flex-col overflow-hidden px-4 sm:px-6 lg:px-16 py-6 md:py-8">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mb-6">
                <div>
                    <h1 className="text-xl sm:text-2xl font-medium text-slate-800 tracking-tight">{t('quotes.title')}</h1>
                    <p className="text-slate-500 text-sm mt-0.5 hidden sm:block">{t('quotes.subtitle')}</p>
                </div>
                <Button onClick={() => setIsModalOpen(true)} variant="default"><FaPlus className="mr-2 h-4 w-4" />{t('quotes.new')}</Button>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-6">
                <KPICard label={t('quotes.kpis.open')} value={baseQuotes.length} icon={<FaFileContract />} />
                <KPICard label={t('quotes.kpis.total_value')} value={totalValue.toLocaleString('de-DE', { style: 'currency', currency: 'EUR' })} icon={<FaEuroSign />} subValue={t('quotes.kpis.total_value_sub')} />
                <KPICard label={t('quotes.kpis.last_7_days')} value={recentCount} icon={<FaClock />} subValue={recentCount > 0 ? t('quotes.kpis.recent_positive') : t('quotes.kpis.recent_empty')} />
            </div>
            <div className="flex-1 min-h-0">
                <DataTable data={filteredQuotes} columns={columns} isLoading={isLoading} searchPlaceholder={t('quotes.search_placeholder')} onExport={handleExport} activeFilterCount={activeFilterCount} onFilterToggle={() => setIsFilterSidebarOpen(v => !v)} isFilterOpen_external={isFilterSidebarOpen} />
            </div>
            <NewProjectModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSubmit={(data: any) => createMutation.mutate(data)} initialData={{ status: 'offer' }} />
